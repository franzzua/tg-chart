...
export function getStayAbscisess(chart: Chart, rect: ClientRect, isMoving, transforms, getX) {
    ...
    return abscisses.map((val, i) => svg(chart, `abs.val.${i}.stay`)`
            <text d transform="${transforms.absciss.getTranslation(getX(val), 0)}" y="${rect.width * .7}" 
...
export function getMoveAbscisess(chart: Chart, rect: ClientRect, isMoving, transforms, getX) {
    ...
    const moveAbscisses = new Array(8).fill(0)
    ...
    return moveAbscisses.map((val, i) => svg(chart, `abs.val.${val.x}.move`)`
        <text d transform="${transforms.absciss.getTranslation(getX(val.date), 0)}" y="${rect.width * .7}" 
